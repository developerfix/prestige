...
class _FormScreenState extends State<FormScreen> {
  ...
  Map<String, DateTime?> week1Dates = {};
  Map<String, DateTime?> week2Dates = {};
  ...
  bool areAllFieldsFilled() {
    ...
    // Check if all the weekdates are filled for both weeks
    if (week1Dates.values.contains(null) || week2Dates.values.contains(null)) {
      return false;
    }
  ...
  // Method to handle date selection for each day
  Future<void> _selectDate(
      BuildContext context, String day, bool isWeek1) async {
    ...
    if (selectedDate != null) {
      setState(() {
        if (isWeek1) {
          week1Dates[day] = selectedDate;
        } else {
          week2Dates[day] = selectedDate;
        }
      });
    }
    // Unfocus the current focus scope (remove focus from the TextFormField)
                              ...
                              Row(
                                children: [
                                  SizedBox(
                                    ...
                                    child: ElevatedButton(
                                      onPressed: () =>
                                          _selectDate(context, day, week == 1),
                                      child: const Text(
                                          'Select Date'), // Display this button for each day
                                    ),
                                  ),
                                  const SizedBox(width: 8),
                                  Text(
                                    week == 1
                                        ? (week1Dates[day] != null
                                            ? DateFormat('y-MM-dd')
                                                .format(week1Dates[day]!)
                                            : 'Select Date')
                                        : (week2Dates[day] != null
                                            ? DateFormat('y-MM-dd')
                                                .format(week2Dates[day]!)
                                            : 'Select Date'),
                                  ),
                                ],
                              ),
                          ...
                          if (areAllFieldsFilled()) {
                            ...
                            showDialog(
                              ...
                              builder: (context) => AlertDialog(
                                ...
                                content: const Text(
                                    'Please fill all the required fields and signatures before the PDF preview.'),
                                actions: [
                                  TextButton(
                      ...
                      onPressed: () async {
                        if (areAllFieldsFilled()) {
                          ...
                          final Email email = Email(
                            body: '',
                            subject: 'Prestige Home Care From Submission',
                            recipients: ['prestigehc.20@gmail.com'],
                          ...
                          showDialog(
                            ...
                            builder: (context) => AlertDialog(
                              title: const Text('Incomplete Form'),
                              content: const Text(
                                  'Please fill all the required fields and signatures before form submission.'),
                              actions: [
                                TextButton(
                                  onPressed: () => Navigator.pop(context),
                                  child: const Text('OK'),
